...
class MemberServiceImplTest {
    ...
    @Mock
    private VerificationServiceCode verificationServiceCode;
    ...
    void changePassword_shouldChangePasswordWhenCodeIsValid() {
        ...
        when(verificationServiceCode.verifyCode(anyLong(), anyString())).thenReturn(true);
        when(memberRepository.findById(anyLong())).thenReturn(Optional.of(member));

        memberService.changePassword(1L, "newPassword", "1234");

        verify(memberRepository, times(1)).save(member);
        verify(verificationServiceCode, times(1)).invalidateCode(1L);
        assertEquals("newPassword", member.getPassword());
    }

    @Test
    void changePassword_shouldThrowExceptionWhenCodeIsInvalid() {
        when(verificationServiceCode.verifyCode(anyLong(), anyString())).thenReturn(false);
    ...
    void requestPasswordChange_shouldSendVerificationCode() {
        ...
        verify(verificationServiceCode, times(1)).sendVerificationCode(member);
    }
